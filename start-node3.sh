...
echo "Creating data directory for Elasticsearch Node 3..."
sudo mkdir -p /var/enc/db/elasticsearch3/data
sudo chown -R 1000:1000 /var/enc/db/elasticsearch3/data # Assuming Elasticsearch runs as UID 1000 in container

echo "Starting Elasticsearch Node 3 using docker-compose..."
sudo docker-compose -f elasticsearch-three-compose.yml up -d

echo "---------------------------------------------------------------------"
echo "Node 3 (elasticsearch-three) startup initiated."
echo "Elasticsearch Node 3 will be available at http://<this_host_ip>:9200"
echo "Ensure ./elasticsearch-three-compose.yml, ./elasticsearch-one-config.yml, and ./log4j2.properties are in the same directory as this script."
echo "---------------------------------------------------------------------"
